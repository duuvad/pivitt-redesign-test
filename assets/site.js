...
(function(){
  ...
  var fine=!window.matchMedia||window.matchMedia('(pointer:fine)').matches;
  // stagger delays
  ['.fgrid','.statband','.steps','.rwork','.svc-list','.imgband','.value','.tgrid','.beliefs'].forEach(function(sel){
...
(function(){
  ...
  var cfg=[
    {ax:56,ay:64,sx:0.00024,sy:0.00030,mx:34,my:26,sc:0.10,ss:0.00034},
    {ax:62,ay:48,sx:0.00020,sy:0.00025,mx:-28,my:-22,sc:0.11,ss:0.00030},
    {ax:46,ay:58,sx:0.00029,sy:0.00022,mx:24,my:32,sc:0.09,ss:0.00038}
  ];
  ...
  function frame(t){
    ...
    for(var i=0;i<blobs.length;i++){var c=cfg[i];
      var dx=Math.sin(t*c.sx+i*2)*c.ax+cmx*c.mx;
      var dy=Math.cos(t*c.sy+i*1.3)*c.ay+cmy*c.my;
      var s=1+Math.sin(t*c.ss+i)*c.sc;
